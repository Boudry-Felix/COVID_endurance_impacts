...
my_results$binary_proportions[c("endurance_sport", "federal_license")]
lapply(
...
my_results$plots$plots_post_covid$dtt2
paste("Reprise immédiate et quelques jours après = court; le reste long")
my_results$binary_proportions$duration_to_training2
...
my_results$plots$plots_post_covid$diff_duration2
paste("Aucun et 1 semaine = court et le reste = long")
my_results$binary_proportions$difficulties_duration2
...
ggplot(
  data = imported_data$raw_data$completed,
  mapping = aes(
    x = imported_data$raw_data$completed$endurance_sport,
    fill = imported_data$raw_data$completed$longer_recuperation
  )
) +
  geom_bar() +
  labs(fill = "longer recuperation") +
  geom_text(aes(label = after_stat(count)),
                    position = position_stack(vjust = 0.5),
                    stat = "count") +
  ggtitle("population_recuperation") +
  xlab("endurance sport")
prop.test(
  table(
    imported_data$raw_data$completed$endurance_sport,
    imported_data$raw_data$completed$longer_recuperation
  ),
  n = NULL,
  correct = FALSE
)
...
my_results$plots$plots_train_diff$time_to_normal_training_volume2
print("Jusqu'à 1 mois = court; le reste = long")
my_results$binary_proportions$time_to_normal_training_volume2
```
...
```{r}
my_table(imported_data$my_colnames)
```
